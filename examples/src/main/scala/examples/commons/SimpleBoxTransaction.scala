...
import examples.hybrid.transaction.mandatory.{PaymentTransaction, PaymentTransactionCompanion, PenaltyTransaction, PenaltyTransactionCompanion}
...
object SimpleBoxTransactionCompanion extends Serializer[SimpleBoxTransaction] {
  ...
  override def toBytes(m: SimpleBoxTransaction): Array[Byte] = {
    m match {
      ...
      case t: PenaltyTransaction => Bytes.concat(Array(m.transactionTypeId), PenaltyTransactionCompanion.toBytes(t))
  ...
  override def parseBytes(bytes: Array[Byte]): Try[SimpleBoxTransaction] = {
    ...
    transactionTypeId match {
      ...
      case PenaltyTransaction.TransactionTypeId => PenaltyTransactionCompanion.parseBytes(bytes.drop(1))
