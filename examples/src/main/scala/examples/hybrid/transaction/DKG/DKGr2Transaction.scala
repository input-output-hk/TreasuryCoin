...
import examples.hybrid.transaction.{DKGr2TxTypeId, SignedTreasuryTransaction}
...
import treasury.crypto.keygen.datastructures.round2.{R2Data, R2DataSerializer}

import scala.util.Try

case class DKGr2Transaction( r2Data: R2Data,
  ...
  override val transactionTypeId: ModifierTypeId = DKGr2Transaction.TransactionTypeId
  ...
  override def toString: String = s"DKGr2Transaction (${json.noSpaces})"
}

object DKGr2Transaction {

  val TransactionTypeId: scorex.core.ModifierTypeId = DKGr2TxTypeId

  def create(privKey: PrivateKey25519,
             r2Data: R2Data,
             epochID: Long): Try[DKGr2Transaction] = Try {
    val timestamp = System.currentTimeMillis()
    val fakeSig = Signature25519(Signature @@ Array[Byte]())
    val unsigned = DKGr2Transaction(r2Data, epochID, privKey.publicImage, fakeSig, timestamp)
    val sig = PrivateKey25519Companion.sign(privKey, unsigned.messageToSign)

    DKGr2Transaction(r2Data, epochID, privKey.publicImage, sig, timestamp)
  }
}

object DKGr2TransactionCompanion extends Serializer[DKGr2Transaction] {

  def toBytes(t: DKGr2Transaction): Array[Byte] = {

    Bytes.concat(
      Ints.toByteArray(t.r2Data.size),
      t.r2Data.bytes,
  ...
  def parseBytes(bytes: Array[Byte]): Try[DKGr2Transaction] = Try {

    var offset = 0
    def offsetPlus (i: Int): Int = { offset += i; offset }

    val r2DataSize = Ints.fromByteArray(bytes.slice(offset, offsetPlus(4)))

    val r2Data = R2DataSerializer.parseBytes(bytes.slice(offset, offsetPlus(r2DataSize)), TreasuryManager.cs).get
    ...
    DKGr2Transaction(r2Data, epochID, pubKey, sig, timestamp)
  }
}
